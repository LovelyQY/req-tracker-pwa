...
| 12 | 批次 185 i18n（A·B·C 完成；D 待做） | 全站 6 语言（6 份全量字典 + 引擎已落地） | 🔶 A·B·C 完成 / D 待做 | 1.3.88 |
| 末 | 阶段 0.6 各模块 cloud 适配层 | 写接入同步队列 + 媒体云存储 | ⬜ 待做（用户指令：排所有批次之后；**当前下一个**） | — |

**当前进度：12 / 13 主项完成（185-A·B·C 已落地，6 份全量字典就绪；仅剩 185-D 全站 rewire；按用户指令，下一阶段为 0.6）。**
...
## 批次 185：i18n 全站多语言（最大单项工程）🔶 A·B·C 完成（v1.3.88）/ D 待做
...
2. **发版登记**：新增 `page-icons` key、`i18n/*.js`（6 份字典 zh-CN/en/zh-HK/zh-TW/ko/ja + 引擎，批次185-A/B/C）、`cloud-storage.js`/`cloud-adapter.js`（0.6 适配层，已在 index.html 加载但写入同步待 0.6 完成 activate）、任何带 `?v=` 的新资源都必须在 `release.sh` 登记，否则发版自检「漂移」失败（批次 169 / 180 教训）。
