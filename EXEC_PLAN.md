...
| 8 | 批次 181 日历框架 + 打卡 + 节假日 | 月历 / 上下班打卡 / 节假日 / 手动调休 | ⬜ 待做（下一个） | — |
| 9 | 批次 182 按小时请假 | 请假面板 / 时长自动算 | ⬜ 待做 | — |
| 10 | 批次 183 当日详情聚合 | 当日 任务/待办/反馈 三栏 | ⬜ 待做 | — |
| 11 | 批次 184 统计报表 | 日 / 周 / 综合 统计 | ⬜ 待做 | — |
...
## 批次 181：日历 TAB 框架 + 上下班打卡 + 节假日/补假 + 手动调休 ⬜

- [ ] `index.html/app.js`：「日历」TAB 已由 180 建好按钮与 `#view-calendar` 空容器；本批实现 `renderCalendar()`。
- [ ] **月历渲染**：网格月视图，每天格子显示日期 + 打卡状态点（已打卡/请假/休息）。
- [ ] **顶部打卡区**：当日「上班打卡」「下班打卡」按钮，记录 `clockIn/clockOut` 时间戳（写 `attendance`）。
- [ ] **节假日/补假**：加载 `holidays-*.json`（年度静态 JSON，每年手动更新一份），标记法定假日(休)/调休补班(班)/补假；周末默认休。
- [ ] **手动调休**：点击某天切换「上班/休息」（`day_override` 覆盖推断，写 `attendance` 记录的 `override` 字段，schema 已预留）。
...
- [ ] 写入 `leave`；日历格子标记请假；工时计算自动扣减（公式：实际工时 = 重叠有效出勤段 − 请假时长）。

---

## 批次 183：点击当天显示当日任务/待办/反馈 ⬜

- [ ] 点击日历某天 → 下方「当日详情」面板，分三栏：任务 / 待办 / 反馈。
- [ ] 按 `createdAt` 日期匹配（若后续加 `planDate/dueDate` 优先用）；按 `RT_PERM` 角色过滤可见范围（本人/团队）；复用 `renderStats` 与现有卡片渲染。
