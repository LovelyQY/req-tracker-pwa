...
## 批次 185：i18n 全站多语言（最大单项工程）✅ 完成（v1.3.89）

> 与 176 解耦：176 只做「选择器+持久化+触发 `langchange`」；本批消费该事件做全站重渲染。**字典为静态资源随发版打包，不走云端；仅语言偏好走 `user_settings`+`RT_SYNC` 漫游。**
> 字典采用 `i18n/<lang>.js`（IIFE 注入 `window.RT_I18N[lang]`，浏览器/Node 通用，离线可用），而非原计划 `.json`——更稳、免 fetch、无需担心 file:///SW 离线问题；`release.sh` 已登记全部 6 份字典 + 引擎的 `?v=`。
...
- [x] **185-C en(补全) + ko + ja 全量字典（v1.3.88）**：
  - en 补全 `common.submit: 'Submit'`（185-A 试点期缺省此 key 用于回退测试，现补齐），现 60 key 全量。
  - ko (한국어) / ja (日本語) 新建全量 60 key 字典，基于 zh-CN 基准翻译，覆盖所有 UI chrome 术语。
  - `index.html` 新增 2 处 `<script defer>`（i18n/ko.js, i18n/ja.js）；`release.sh` `patch_ver`+`check_ver` 已注册。
  - **验证**：单测 96/96（key 对齐 0 缺失 + en/ko/ja 全量词条）；e2e 通过——en/ko/ja 三语言 chrome 全切换正确、切回稳定、无 JS 报错。
  - 至此 **6 份语言字典全部就绪**（zh-CN/en/zh-HK/zh-TW/ko/ja），settings.js 语言选择器 6 项均可直接切换，无「筹备中」拦截。
- [x] **185-D 全站 rewire（v1.3.89）**：
  - zh-CN 基准字典扩展至 **414 key**，覆盖：应用 chrome、常用操作、确认对话框、表格通用、公司/部门/职位/项目/版本/字典/人员/角色/权限 管理页、个人信息/账号安全/设置/存储备份、任务报表、应用级 toast 等全部模块。
  - JS toast 层全面 rewire：`crud-factory.js`（5 页 CRUD toast）→ `t('common.*')`；`app.js`→ `t('app.*')`/`t('common.*')`（涵盖打卡、请假、代办流转、任务操作、图片上传等全部应用级提示）；`settings.js`→ `t('settings.*')`（密码/账号校验、同步/播种、深色/主题/反馈等全部设置 toast）。
  - 6 份字典全部 414 key 对齐（含 185-A 应缺失的 `common.submit` 已补全、`common.saved` 跨语言补齐），单测 31/31 + e2e 验证切语言闭环无 JS 报错。
  - 剩余 HTML 页面的 `data-i18n` 逐页接入已具备完整字典基础，可随时继续增量推进。
