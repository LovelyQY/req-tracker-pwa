...
### A. 渲染与状态 Bug 修复
- **#1 侧边栏默认加载历史数据才跳当前用户**
  - 根因：`index.html` 抽屉(`#drawer`)初始渲染读的是缓存/上次用户，再异步拉当前登录用户并替换，导致首屏闪一下旧数据。
  - 处置：抽屉初始化先以 `RT_SESSION`/当前登录用户渲染，异步补齐；未登录态直接显示占位，不渲染他人数据。
- **#6 切换语言后未转换** ⭐ 最高优先（多数"未翻译"的根因）
  - 根因：`settings.js` 等页面大量中文 **硬编码在 JS 字符串 / 无 `data-i18n` 的 HTML** 中（如 `<div class="set-row-title">昵称</div>`），`langchange` 仅重填 `[data-i18n]` 元素 → 切换后不翻译。此外部分动态渲染内容未走 `t()`。
  - 处置：① `i18n.js` 的 `applyLang` 已具备 `[data-i18n]/[data-i18n-ph]/[data-i18n-aria]` 回填能力，需给所有静态中文补 `data-i18n`；② 所有 `RT_SETTINGS_PAGE`/其它页面动态渲染统一改用 `t(key)`；③ 跨页跳转后 `onDOMReady` 主动 `applyLang(getLang())` 兜底。
...
### C. 深色模式 & 主题色
- **#7 深色模式主题色换成深色（蓝色太突兀）；深色模式下设置页需默认深色，要先切深色才能换回浅色**
  - 根因：深色模式仅切背景，主色仍用亮色蓝，且设置页初始不读深色态。
  - 处置：① 深色模式配套一套"深色主题色"（低饱和深色系），与浅色蓝解耦；② `theme-bootstrap.js` 在启动即根据 `prefers-color-scheme`/已存偏好应用深色，设置页初始即深色态，深浅切换双向可用。
- **#8 主题色：去掉默认自定义颜色输入，改为可选 + 点"恢复默认"才出现**
  - 处置：默认仅展示预设色板；`#themeCustom` 颜色输入默认隐藏，点击"自定义"或"恢复默认"后再按需出现。
...
|------|------|----------|------|--------|---------|
| **186** | i18n 引擎修复 + 侧边栏当前用户优先 | #6, #1 | 修复 | **P0** | v1.3.91 |
| **187** | 深色模式 & 主题色重构 | #7, #8 | 重构 | P1 | v1.3.92 |
