...
(function (root) {
  ...
  function loadReportData() {
    ...
    // batch93: data permission scope filter
    ...
    // 批次70/76：拉取全部生命周期流水并按 todoId 分组，为每张清单卡片附加单行灰时间所需的状态操作行（statusOpLine）
    ...
    return Promise.all(tasks.concat([lcTask])).then(function () {
      // batch93: project→dept cross-filter for todos
  ...
  function buildTodoCardHtml(t) {
    ...
    // 批次79：单行灰时间——会议按 opCode 映射专属标签（创建/会议开始/会议结束/会议取消时间），其余类型用操作码中文名 + '时间'，与首页待办卡片口径一致
