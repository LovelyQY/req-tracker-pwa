...
实现:
1. 页面顶部追加语言切换按钮「中 ⟷ EN」，状态存 localStorage
2. `buildTreeHtml` 根据语言模式渲染：
   - 中文模式：`typeLabelMap = { module:'模块', page:'页面', op:'操作' }`，displayName 取 `(menuName || 注册表中文名 || menuCode)` 的 `.trim()`，隐藏 `.tcode` span
   - 英文模式：类型标签用原始 `n.nodeType`，主名称显示 `menuCode`，`.tname` 改为显示 `menuName`（如存在）作为副标题
3. sheet 表单中节点类型 select option 同步切换中/英文
4. 权限注册表 `getRegistryEntry(code)` 提供中文名供中文模式兜底
...
| D2 | 批次 106 合并问题3+4 — 升级为权限树中英双语模式（中文版只显示名称，英文版显示编号），两模式通过页面按钮切换 |
| D3 | 批次 107 独立处理 — 涉及新增逻辑改造，影响面较大 |
| D4 | 批次 108 独立处理 — 涉及删除数据层操作，需谨慎 |
| D5 | 批次 109 独立处理 — 纯文字替换，极简改动 |
| D6 | 中英双语模式下，中文版默认显示 menuName/注册表中文名，英文版默认显示 menuCode；不再单独处理 menuName 空值问题 |
