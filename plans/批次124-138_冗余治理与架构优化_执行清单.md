...
- **实测规模：14 个文件含监听**（12 个 HTML + `permission.js` + `role.js`），非计划假设的 12/11 文件、23 处。全部经 `addEventListener` 注册（无内联 `onpageshow=` 属性）。
- **实测性质（推翻“高度一致可复用”假设）：**
  - **统一“渲染模式”仅 11 处**：`company / department / role / position / project / project-version / security / profile-detail / profile / user / dictionary` —— `pageshow` 与 `visibilitychange`(可见时) 调用**同一函数**（`render`/`renderProfile`/`refresh`/`renderTypeSelect()+render()`）。
  - **3 处语义分歧，无法用同一 `onVisible(fn)` 统一**：
    1. `index.html` 抽屉：`pageshow`→`refreshDrawerUser()+maybeReopenDrawer()`；`visibilitychange`→仅 `refreshDrawerUser()`（**少一步**）。
    2. `permission.js`：`pageshow`→`load(true)`；`visibilitychange`→`load(false)`（**参数不同**）。
    3. `index.html:790` 版本检查：独立 `visibilitychange` 箭头函数 `if(!document.hidden) checkRemoteVersion()`（用 `document.hidden`、非渲染模式）。
  - `status.html` 的 `pageshow` 为 bfcache `e.persisted` 检查 + `version.json` 比对重载（**非渲染模式**），且无 `visibilitychange`。
- **决策：延期**（符合 YAGNI / 低版本号风险原则）：
  - 即便对 11 处统一模式，抽取 `onPageShow(fn)/onVisible(fn)` 后**每页仍是 2 行**（2 行样板 → 2 行调用），**净行数节省 ≈ 0**；唯一收益是把 `if (visibilityState==='visible')` 守卫集中到一处。
  - 部署需全局可用：无语义契合的“全页加载”文件（`auth/config/db` 语义不符；`ui-utils.js` 未加载于 `index / profile / status / profile-detail` 4 页）。无论并入 `auth.js`（污染）还是新增 `visibility.js`（12-14 处 script 标签 + release.sh + sw.js 开销），都为“零净收益”付出可观开销。
  - 对 3 处分歧站点无法干净复用，反而需保留各自内联包裹，徒增间接层。
  - **属“为抽象而抽象”，本批次不执行**；建议未来确有统一守卫需求时再独立重构。
...
## 5. 实测执行记录（批次138）· 无操作（no-op）

- **实测：`kill-sw.html` 不存在。** 工作区无该文件；`git ls-files` 无结果（未纳入跟踪）；全仓 `grep 'kill-sw'` 除自身外零外部引用；`release.sh` / `sw.js` / `manifest.json` 均未登记。
- **推测已在更早批次（如 131/132 清理）中被删除**，目标文件已不存在，**本批次无操作**。
- **备注（未改动）**：`about.html` / `index.html` 仍含 `serviceWorker.getRegistrations()` / `unregister()` 的“注销 SW”功能，属正常设置项，非孤立 `kill-sw.html`，不在本批次范围。
