...
- [ ] 新增 `permissions-registry.js`：`RT_PERM_REGISTRY`（§1.2 结构；**全部 snake_case 命名**；看板拆 `page_board_task` + 三个 `page_board_todo_*`；报表拆四个 `page_report_*`）。
- [ ] **对照字典表补全操作按钮（完整性基线，用户重点要求）**：逐一核对 `app.js` / `task-lifecycles.js` / `todo-lifecycles.js` 中出现的真实按钮，确保注册表叶子齐全：
  - [ ] `TASK_OPERATION`（8）：`DEV_SUBMIT / TEST_START / PAUSE / RESUME / TEST_DONE / ONLINE / RESET / DELETE` → `op_task_*`（任务区）。
  - [ ] `TODO_OPERATION`（10）：`TODO_CREATE / EDIT / START / COMPLETE / HANDOFF / ONLINE / DELETE / CANCEL / END / RESET`；**但三种代办类型按钮不同**，须按 `app.js` `getTodoActions` 对齐：任务事项=`start/complete/edit/del/reset`、缺陷=`start/complete/handoff/online/edit/del/reset`、会议=`start/cancel/end/edit/del/reset`；三者 `op_todo_*` 集合各异（非各一套相同）。
  - [ ] `TODO_TYPE`（3）：`TASK_ITEM / BUG / MEETING` → 三个 `page_board_todo_*` 页面。
  - [ ] 基础操作 `op_view/create/edit/delete/export/assign_role/enable` 落在对应页面。
  - [ ] 任何 `app.js` 里实际渲染、但注册表缺漏的按钮，补进注册表（保持「已配置」可守卫）。
- [ ] `flattenRegistryCodes()` 展开全部有效 code 集合；`isCodeConfigured(code)` 判定「已配置」。
- [ ] `seedMenusFromRegistry()`：按注册表幂等播种 `menus`（module/page/op 三级节点，`parentCode` 链式指向父 `menuCode`、`menuCode` 去重、缺则补、已有跳过）。
- [ ] `menus` 树构建 `buildMenuTree()`（复用 `departments.buildHierarchy` 思路；按 `parentCode` 关联）。
- [ ] 单测 `tests/test-batch82-registry.js`：注册表展开 code 集合正确（含任务区 8 个 `op_task_*` + 代办三类型各异 + 报表 4×2 等）；种子幂等（跑两次行数不变）；`isCodeConfigured` 对注册表内外 code 判定正确；树形 `parentCode` 正确；代办三类型页独立。
- [ ] **注册表为权限码单一真相（§1.8 强规则）**：本批次起，`PERMISSION_REGISTRY` 是新增页面/按钮权限的唯一登记处；后续所有批次新增页面或操作按钮，均须登记注册表并接线 `data-perm`（见 §1.8）。

#### 批次 83 — 运行时解析 RT_PERM
- [ ] `permissions.js` 暴露 `RT_PERM`（§1.4 API）：`can/canAny/canAll/getMenuCodes/isAdmin/getDataScope`。
- [ ] `can` 逻辑：`isAdmin` 短路 true → 否则取 `getMenuCodes(account)` 且对应 `menu.enabled` 为 true → 命中返回 true（落实 §1.2 停用优先、§1.5 admin 全部）。
- [ ] 会话缓存：`cachePermissions(account)` / `clearPermissionCache()`（登录成功与角色变更时调用）。
- [ ] 单测 `tests/test-batch83-rtperm.js`：admin 全 true；普通用户按 `users.roleIds→roles.menuCodes` 命中；停用菜单 `can` 返回 false；`getDataScope` 返回 `{deptId, includeSub:true}`。

#### 批次 84 — 系统管理员默认角色播种 + 启动串联
- [ ] `users.js` 新增 `ensureDefaultAdminRole()`（§1.6，幂等）：建「系统管理员」角色（`isSystemAdmin=true`、`menuCodes`=全部注册表 code）、绑定 `admin.roleIds`。
- [ ] 在登录成功 / 应用启动入口串联：`ensureDefaultAdmin()` → `ensureDefaultAdminRole()` → `seedMenusFromRegistry()`（与现有 `migrateAccounts()` 同位置）。
- [ ] D3 已确认不考虑老用户：当前仅有 `admin`，seed 保证其获系统管理员角色；`users.roleIds` 缺省空数组即可，无需兜底逻辑。
- [ ] 单测 `tests/test-batch84-seed.js`：幂等（多次调用角色/菜单/绑定不重复）；`admin` 经 `RT_PERM.isAdmin` 与 `can(任意code)` 为 true。

### 阶段 1：管理页面（基础数据下钻）

#### 批次 85 — 基础数据页入口
- [ ] `basic-data.html` 的 `MODULES` 在「项目版本管理」「字典管理」后追加 `角色管理(role.html)`、`权限管理(permission.html)`（按现有 MODULE 结构 + icon）。
- [ ] 两页 `<script src="...">` 均带 `?v=` 并在 `release.sh` 注册（RULES 缓存破坏 + 自检）。

#### 批次 86 — 角色管理页（role.html + role.js）
- [ ] 列表：角色名称、启停状态、权限数；新增/编辑/启停（遵循 `departments` 页模式 + 返回栈 `goBack`/`navTo`）。
- [ ] 分配权限：树形勾选（来自 `buildMenuTree`，仅允许勾选**已配置**节点；未配置项禁用勾选，呼应需求「只可勾选已配置的权限」）。
- [ ] 保存：`saveRolePermissions`（追加写历史）+ 覆盖 `roles.menuCodes`；系统管理员角色禁停用/禁删。
- [ ] 角色列表/详情**展示「引用的人」清单**（按 `users.roleIds` 反查当前引用本角色的人员，隐藏 32 位 id，显示工号/姓名）；**点击可查看引用该角色的人员**（弹窗 / 抽屉列出人员，或跳 `user.html?roleId=...` 带角色筛选），便于审计谁拥有此角色（D4 增强）。
- [ ] **删除前校验「无人员引用」方可删**（D4）：仍有引用时禁用删除并提示引用人数；系统管理员角色始终禁删。
- [ ] 表单遵循 RULES：不展示 32 位 id；`-webkit-tap-highlight-color` reset；返回用 `goBack()`。

#### 批次 87 — 权限管理页（permission.html + permission.js）
- [ ] 树形展示页面权限按钮（module/page/op 三级，`buildMenuTree`）。
- [ ] 新增/编辑/启停权限节点（`createMenu/updateMenu`，`menuCode` 唯一校验、防环）；启停写 `enabled`。
- [ ] **徽标**：每节点显示「已配置（绿）」/「新增未真正配置（灰/警示）」，来源 `isCodeConfigured(menuCode)`。
- [ ] 仅管理员可见（入口受 `RT_PERM.can('page_perm_view')` 守卫，批次 89 接线；本批次先完成页面）。

#### 批次 88 — 人员管理增强（user.html + users.js）
- [ ] 人员表单新增「分配角色」多选（来自 `roles` 列表，仅启用角色可选）。
- [ ] 保存：`saveUserRoles`（追加写 `user_role` 历史）+ 覆盖 `users.roleIds`；`validatePerson` 兼容 `roleIds` 选填。
- [ ] 列表展示人员已分配角色名称（按 `roleIds` 解析，隐藏 32 位 id）。

### 阶段 2：运行时强制执行（按钮级）

#### 批次 89 — 守卫机制 + 首页/看板接线
- [ ] `RT_PERM.guard(root)`：扫描 `[data-perm]`，无权限则 `style.display='none'`（或加 `.perm-hidden`）。
- [ ] 各入口页 `<script src="permissions.js?v=X">` + `permissions-registry.js`（release.sh 注册）。
- [ ] `app.js` 首页/看板：按钮加 `data-perm`（如 `op_board_task_view` / `op_task_dev_submit` / `op_todo_complete`，snake_case，按 §1.2 注册表）；渲染后 `RT_PERM.guard()`；空态按 `can` 显式「无权限」。
- [ ] 登录成功后 `cachePermissions(account)`。

#### 批次 90 — 基础数据各页接线
- [ ] 公司/部门/职位/项目/版本/字典页：增删改按钮 + 入口加 `data-perm`；渲染后 `guard()`。
- [ ] 角色/权限/人员管理页自身入口受对应 `page:*:view` 守卫（仅管理员可见）。

#### 批次 91 — 报表/个人/系统页接线 + 抽屉收起
- [ ] 统计报表明细页（report-task / report-bug / report-todo / report-meeting）各受自身 `page_report_*` 守卫；导出按钮 `op_export` 单独守卫（四个子页各自独立权限）。
- [ ] 个人信息/账号安全/存储备份/关于：按钮级 `data-perm` + `guard()`。
- [ ] `index.html` 抽屉：无 `view` 权限的入口隐藏（如非管理员看不到「角色管理」「权限管理」；无 `page_report_*` 则隐藏对应统计入口）。

### 阶段 3：数据权限（部门隔离，最高风险）

#### 批次 92 — 数据权限核心
- [ ] `getDataScope(account)` 计算部门子树（自身 + 下级，递归 `departments.parentId`）。
- [ ] 数据层 read 过滤钩子：在 `getAllUsers/getAllDepartments/getAllProjects/requirementTasks` 等列表函数内，非管理员按 `deptId ∈ 子树` 过滤（管理员跳过，D6）。
- [ ] 公司/部门页：按可见部门过滤其下属数据。
- [ ] featureFlag（可选灰度）：`RT_CONFIG.featureFlags.dataPermission` 默认开；便于回滚。

#### 批次 93 — 报表/统计过滤 + 一致性核验
- [ ] 报表（report-common.js / report-*.js）按 `getDataScope` 过滤统计范围。
- [ ] 跨页一致性核验：同账号在各页看到的数据范围一致；管理员可见全部。

### 阶段 4：收尾

#### 批次 94 — 文档 + 全量单测 + 发版
- [ ] `DB_SCHEMA.md` 增补 §2 四张表（沿用现有风格）。
- [ ] `RULES.md` 增补权限规则（角色/权限/数据权限概念、admin 默认角色，**以及强规则 §1.8：新增页面/按钮必须登记 `PERMISSION_REGISTRY` 并接线 `data-perm`**）；`release.sh` 发版自检新增「扫描 `data-perm` 取值，未登记注册表则阻断发版」；关于页补充权限说明。
- [ ] 全量 `node --test tests/test-batch*.js` 通过；`node --check` 全部改动文件通过。
- [ ] 浏览器核验：admin 全权限；普通角色按配置；数据权限按部门；历史表 append-only 验证（改角色/人员后旧 snapshot 仍在）。
