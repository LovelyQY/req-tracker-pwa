...
- **工作流**：设置页提供「初始化示例」入口，直接生成需求中的 5 个工作流及模板（实现见 Batch 230）。
...
| 223 | v1.4.28 | 首页空状态统一 | 2 |
| 224 | v1.4.29 | 日历与考勤（上）：清理/周末假期/打卡分上下午 | 4 |
| 225 | v1.4.30 | 日历与考勤（下）：颜色统一/事件类型/云端时间 | 5 |
| 226 | v1.4.31 | 设置清理与个人信息 | 4 |
| 227 | v1.4.32 | 设置展示与反馈 | 5 |
| 228 | v1.4.33 | 导出（本地/邮箱/腾讯文档）与初始化示例入口 | 5 |
| 229 | v1.4.34 | 表单模板增强 | 5 |
| 230 | v1.4.35 | 工作流增强：职位审批 + 直接生成工作流 | 2 |
| 231 | v1.4.36 | 日历与流程关联 | 2 |
...
## 六、Batch 221（v1.4.26）— 首页头部：应用名 / 通知 / 城市
...
## 十、Batch 225（v1.4.30）— 日历与考勤（下）：颜色统一 / 事件类型 / 云端时间
...
## 十一、Batch 226（v1.4.31）— 设置清理与个人信息
...
## 十三、Batch 228（v1.4.33）— 导出（本地/邮箱/腾讯文档）与初始化示例入口
...
5. **设置页「初始化示例」入口**：新增入口（触发 Batch 230 直接生成 5 个工作流及模板；入口文案见 i18n `settings.initExamples`），仅做入口与调用，工作流与模板的真实生成逻辑在 Batch 230。

## 十四、Batch 229（v1.4.34）— 表单模板增强
1. 表单模板字段类型增加「单选框」。
...
## 十五、Batch 230（v1.4.35）— 工作流增强：职位审批 + 直接生成工作流
...
- **导出到本地**：文件直接存到你手机 / 电脑的「下载」里，马上能打开用。最简单、不依赖网络服务端，**本批次（Batch 228）内完整完成**。
- **导出到个人邮箱（统一方法，推荐）**：你在设置里只填一个「接收邮箱」（QQ / 163 / 任意），系统把 Excel 作为附件发到这个邮箱，电脑上就能收。
  ...
  - **可行性**：本批次（Batch 228）内可完成「弹框 + 接收邮箱字段 + 云端发信函数」前端与骨架；**已确认云端具备发信能力（SMTP/SES 已配好）**，本期即可直接真实发信，无需「凭证未就绪」主路径。代码中仍保留「邮箱服务未配置」兜底分支作为健壮性（极端情况下云端发信失败提示）。
