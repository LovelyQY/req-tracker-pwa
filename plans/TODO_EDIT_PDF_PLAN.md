...
> 测试环境：Chrome/Edge 桌面端。先确认 IndexedDB 中有各类型代办数据（可通过首页「代办」TAB 新增/编辑），再逐项验证。

### 批次 45：待办编辑页全屏

| # | 测试项 | 操作 | 预期结果 |
|---|--------|------|----------|
| 1 | 新增代办全屏 | 首页切换到「代办」→ 点击右下角 + 按钮 | 弹窗撑满全屏（`100vw×100dvh`，无圆角）；nav-bar 标题"新增代办"；表单从顶到底滚动 |
| 2 | 编辑代办全屏 | 点击任意代办卡片上的「编辑」按钮 | 同 #1 全屏效果；标题"编辑代办"；字段回填正确 |
| 3 | 类型切换字段显隐 | 新增时，点击类型 chips 切换 TASK_ITEM / BUG / MEETING | 每切换一次，表单字段组正确显隐（desc vs name、tf-bug vs tf-meeting vs tf-time-*） |
| 4 | 返回按钮关闭 | 打开编辑页 → 点击左上角 ← 返回按钮 | 弹窗关闭（`.show` 移除），回到代办列表；body 滚动恢复 |
| 5 | 取消按钮关闭 | 打开编辑页 → 点击底部「取消」 | 同 #4 |
| 6 | 保存后关闭 | 编辑已有代办，修改任意字段 → 保存 | toast"已保存"，弹窗关闭，列表刷新显示修改 |
| 7 | 滚动可见 footer | 编辑页表单字段较多时滚动到底 | `.modal-footer`（保存/取消）始终可见（`position:sticky;bottom:0`） |
| 8 | Modal 动画 | 打开/关闭编辑页 | 打开时有 slide-up (`translateY(12px)→0`)，关闭瞬间消失 |

### 批次 46a：状态变更写时间到 todo

> 前置：打开 Chrome DevTools → Application → IndexedDB → `req-tracker` → `todos`，找到对应记录查看。

| # | 测试项 | 操作 | 预期结果 |
|---|--------|------|----------|
| 1 | TASK_ITEM 开始处理 | 新建 TASK_ITEM → 点击卡片「开始处理」 | `todos` 记录中 `startTime` 不为 null，`startBy` 为当前用户 |
| 2 | TASK_ITEM 完成 | 对处理中的 TASK_ITEM 点击「完成」 | `completeTime` + `completeBy` 写入，`startTime` 保持不变 |
| 3 | BUG 全部流转 | BUG: 开始处理 → 完成 → 再新建一个 BUG 并 开始处理 → 转交 → 上线 | 每个操作触发后：`startTime` / `handoffTime` / `onlineTime` 分别写入正确的时间戳 |
| 4 | MEETING 开始→结束 | 新建会议 → 开始 → 结束 | `startTime` 写入；结束后 `completeTime` 写入（会议结束用 completeTime 存储） |
| 5 | MEETING 取消 | 新建会议 → 取消（填原因） | `cancelTime` + `cancelBy` + `cancelReason` 写入（已有逻辑） |
| 6 | 重复操作 | 对同一记录连续点击两次「开始处理」 | 第二次 `startTime` 覆盖第一次，取最新值 |
| 7 | 旧记录兼容 | 打开批次 46a 之前的旧代办编辑页 | 旧记录无 `startTime` 等字段，编辑页时间字段为空（不报错） |

### 批次 46b：表单开放时间字段

| # | 测试项 | 操作 | 预期结果 |
|---|--------|------|----------|
| 1 | TASK_ITEM 时间字段 | 编辑一个 TASK_ITEM | 显示「时间信息」组：创建时间（只读文本）+ 开始处理（datetime-local）+ 完成（datetime-local） |
| 2 | BUG 时间字段 | 编辑一个 BUG | 「时间信息」组含：创建时间 + 开始处理 + 完成 + 转交 + 上线（5 个可编辑字段）；「反馈时间」仍在 tf-bug 区块中 |
| 3 | MEETING 时间字段 | 编辑一个 MEETING | 「时间信息」组含：创建时间 + 开始 + 结束 + 取消（4 个可编辑字段）；「会议时间」仍在 tf-meeting 区块中 |
| 4 | 类型切换隐藏 | 编辑页切换类型 chips | 时间字段组随类型切换正确显隐（tf-time-taskitem/bug/meeting） |
| 5 | 默认值为流转时间 | 先对代办执行「开始处理」→ 再打开编辑 | 「开始处理」默认值为刚才点击按钮的时间（格式 `YYYY-MM-DDTHH:mm`） |
| 6 | 手动修改后持久化 | 修改「开始处理」时间 → 保存 → 刷新 → 再打开编辑 | 显示修改后的时间，未被流转记录覆盖 |
| 7 | 清空时间 | 将「开始处理」清空 → 保存 → 再打开 | 显示为空（input 无 value） |
| 8 | 创建时间只读 | 编辑任何类型代办 | 创建时间显示为纯文本（`yyyy-MM-dd HH:mm` 格式），无 input 框 |
| 9 | 新增代办默认 | 点击 + 新增代办 | 所有时间字段为空（除会议时间/反馈时间等已有默认的可选字段） |
| 10 | 新建后流转再编辑 | 新建 TASK_ITEM → 开始处理 → 完成 → 编辑 | 「开始处理」「完成」均显示对应流转时间 |

### 批次 47：导出PDF横版表格

| # | 测试项 | 操作 | 预期结果 |
|---|--------|------|----------|
| 1 | 任务统计 PDF | 进入 report-task.html → 点击「导出PDF」 | 打印预览：横版 A4；统计卡 + 模块图 + 12 列表格（名称/类型/优先级/状态/项目/版本/描述/开发提交时间/测试开始时间/测试结束时间/上线时间/工时） |
| 2 | 任务事项 PDF | 进入 report-todo.html → 点击「导出PDF」 | 打印预览：横版 A4；10 列表格（描述/类型/状态/项目/版本/关联开发/备注/创建时间/开始处理时间/完成时间） |
| 3 | 缺陷追踪 PDF | 进入 report-bug.html → 点击「导出PDF」 | 打印预览：横版 A4；13 列表格（含反馈人/反馈时间/备注/开始处理时间/完成时间/转交时间/上线时间） |
| 4 | 会议 PDF | 进入 report-meeting.html → 点击「导出PDF」 | 打印预览：横版 A4；13 列表格（含会议地点/会议纪要/备注/开始时间/结束时间/取消时间/取消原因） |
| 5 | 无主 ID | 检查任意 PDF 表格 | 不出现 32 位 hex 字符串（如 `a1b2c3d4...`）；项目/版本已转为中文名 |
| 6 | 屏幕不显示表格 | 在报表页正常浏览 | `.rf-detail-table` 不可见（`display:none`） |
| 7 | 打印隐藏按钮 | PDF 预览中检查 | `rm-list-btn`（任务清单按钮）和 `tl-overlay` 均不可见；`nav-bar` 也不可见 |
| 8 | 空数据导出 | 筛选一个无数据的时间段 → 导出 PDF | 表格不显示（`display:none`），仅保留统计卡和图 |
| 9 | 时间格式化 | 检查表格中的时间列 | 时间格式为 `yyyy-MM-dd HH:mm`（非时间戳数字） |
| 10 | 工时列 | 任务统计 PDF 中检查「工时」列 | 显示格式如 `12.5H`，非纯数字 |

### 批次 48：报表卡片按项目分布

| # | 测试项 | 操作 | 预期结果 |
|---|--------|------|----------|
| 1 | todo 项目卡片 | 进入 report-todo.html，确保有 ≥2 个项目的数据 | 每个项目一张卡片，按条目数降序排列；卡片内 3 格（未处理/处理中/已完成）含数字+色条 |
| 2 | bug 项目卡片 | 进入 report-bug.html | 每个项目一张卡片，5 格状态分布（含未处理/处理中/已完成/待开发/已上线），数字着色取自 bugStatusColor |
| 3 | meeting 项目卡片 | 进入 report-meeting.html | 每个项目一张卡片，3 格状态分布（未开始/已结束/已取消） |
| 4 | 清单按钮跨状态 | 点击任意项目卡片的「任务清单(N)」按钮 | tl-overlay 打开，列出该项目**全部**记录（不限状态），标题显示项目名 |
| 5 | 清单按钮数字 | 检查卡片上的「任务清单(N)」 | N = 该项目在筛选范围内的 total 记录数 |
| 6 | 色条百分比 | 检查状态色条 | 每个状态色条的宽度 = `该状态数/该项目总数 × 100%`，各条之和可能 >100%（因为各状态独立统计）|
| 7 | 项目排序 | 检查卡片排列顺序 | 按项目条目数降序（最多的项目排第一） |
| 8 | 未指定项目 | 创建一个 projectId 为空的代办 → 查看报表 | 显示「(未指定项目)」卡片 |
| 9 | 空数据 | 筛选无数据时段 | 显示 "📭 该范围暂无数据" |
| 10 | 时间筛选刷新 | 修改年/季/月筛选 | 项目卡片即时刷新，统计数字和色条重新计算 |
| 11 | bug 关联任务统计 | 在 report-bug.html 中检查 | 在项目卡片下方，「关联任务统计」区域仍正常展示（不受批次 48 影响） |
| 12 | 任务统计不变 | 进入 report-task.html | 布局保持不变（仍为原状态模块+bar 形式，未受批次 48 改动） |

---

> 📋 测试执行建议：按批次顺序执行（45→46a→46b→47→48），每批次全部通过后再进入下一批。
> 共 45 项测试（45:8 + 46a:7 + 46b:10 + 47:10 + 48:12）。
