...
- 禁止出现 `U+FFFD`（`\xef\xbf\xbd`）替换字符——这是编码损坏的标志
- `.html`、`.js`、`.css`、`.md`、`.json` 全部适用
- `pre-commit` hook 自动扫描暂存文件，检测到 `U+FFFD` 则拒绝提交
- 若遇到中文无法保存：编辑器 → 另存为 → 选择 "UTF-8 without BOM"
- 历史 U+FFFD 数据已在批次 95 全部清理（4 文件 146 处），此后不允许再引入

---

## UI 元素 × 权限 × 六语言 配置台账同步规则

**配置文件**：`plans/UI_PERM_I18N_MANIFEST.md` 是 PWA 全页面 UI 元素 × 权限 × 六语言的**唯一配置台账**，覆盖整个 PWA 的所有页面模块（38 个 html），逐页登记：**UI 元素 → 权限 key（`data-perm`）→ i18n key（六语言 zh-CN / zh-TW / en / ja / ko / ar）**。

**强规则（强制同步）**：任何页面 / UI 元素的**修改或新增**，必须**同步修改这份台账**——

- 涉及的文案 / 按钮 / 菜单 / 弹框 / 图例 / 状态名 / 提示等，在 `UI_PERM_I18N_MANIFEST.md` 对应页面补一行；
- 同步在 `permissions.js`（`RT_PERM_REGISTRY`）补 `data-perm` 权限码（见上方「§1.8 权限注册表强规则」）；
- 同步在 `i18n.js` + 6 语言文件补 i18n key，不得残留硬编码中文（六语言全覆盖）；
- 台账行状态随实现回填（`✅` 已覆盖 / `⛔` 缺权限 / `⚠️语` 缺语言 / `🔲` 待登记 / `🔍` 待核对）。

**用途**：落实权限校验全覆盖与六语言全覆盖；最后统一核对以该台账为准——漏权限 / 漏语言，扫表即知。

**自检（建议）**：发版或全量 `node --test` 时附带 grep 硬编码中文 + 未注册 `data-perm`，与台账交叉验证，不一致的报错阻断。
